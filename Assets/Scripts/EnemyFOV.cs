...
public class EnemyFOV : MonoBehaviour {
    ...
    void FindVisibleTargets() {
        ...
        for (int i = 0; i < targetsInViewRadius.Length; i++) {
            ...
            if (Vector3.Angle((boss.bIsGoingRight) ? transform.right : Vector3.left, dirToTarget) < viewAngle / 2) {

                float dstToTarget = Vector3.Distance(transform.position, target.position);
                //If line draw from object to target is not interrupted by wall, add target to list of visible 
                //targets
                if (!Physics2D.Raycast(transform.position, dirToTarget, dstToTarget, obstacleMask)) {
                    Debug.Log("no alcance");
                    boss.followPlayer = true;
                    visibleTargets.Add(target);
                }else{
                    Debug.Log("fora do alcance");
                    boss.followPlayer = false;
                }
            }
        }
    }
